...
def get_model(app_label, model_name):
    ...
    except AppRegistryNotReady:     # djangoで用意されているエラー
...
def is_model_registered(app_label, model_name):
    """
    特定のモデルが登録されているかどうかを確認するメソッド。
    オスカーモデルがフォーク型アプリによって上書きされない場合にのみ登録するために使用される。
    """
    try:
