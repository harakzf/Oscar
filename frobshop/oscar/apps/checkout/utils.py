class CheckoutSessionData(object):
    """
    すべてのチェックアウトセッションデータを整列化するクラス。

    多段階のチェックアウトでは、いくつかのフォームを送信し、それらのデータを最終注文が確定するまで保持する必要があります。
    このクラスは、最終注文の書き出しが必要になるまで、チェックアウトフォームのデータを保存および整理するのに役立ちます。
    """
    ...
    def __init__(self, request):
        ...
        if self.SESSION_KEY not in self.request.session:
            self.request.session[self.SESSION_KEY] = {}                 # セッションに新規ディクショナリ作成【checkout_data = {}】

    def _check_namespace(self, namespace):
        """
        セッション辞書内のネームスペースが初期化されていることを確認するメソッド
        """
    ...
    def set_order_number(self, order_number):
        self._set('submission', 'order_number', order_number)       # 'submission':namespace, 'order_number':セッションキー,order_number:実際の値
