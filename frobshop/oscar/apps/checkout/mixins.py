...
class OrderPlacementMixin(CheckoutSessionMixin):
    ...
    def handle_successful_order(self, order):
        """
        注文が正常に行われた後に必要なさまざまな手順を処理するメソッド。

        （注文が送信されたときにカスタムアクションを実行する場合は、このビューをオーバーライドする。）
        """
        # Send confirmation message (normally an email)
        self.send_confirmation_message(order, self.communication_type_code)

        # セッションデータのフラッシュ→'checkout_data'キーで指定されたデータを消去
        self.checkout_session.flush()

        # セッション内で注文IDを保存←お礼のページ（thank_you.html）に読み込むことができるようにするため
        self.request.session['checkout_order_id'] = order.id        # セッションに「checkout_order_id」キーでオーダーIDを登録（8とか9とか）
