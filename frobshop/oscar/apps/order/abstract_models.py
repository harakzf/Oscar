...
from decimal import Decimal as D                                # pythonで10進数を扱うためのライブラリ
...
class AbstractOrder(models.Model):
    ...
    def basket_total_before_discounts_excl_tax(self):
        """
        割引を適用する前の、税を除いたバスケットの合計額を返す
        """
        total = D('0.00')                                           # 変数「total」に初期値'0'（10進数）を代入
    ...
    class Meta:
        abstract = True
        app_label = 'order' # モデルが所属するアプリケーションの名前を指定
...
class AbstractOrderNote(models.Model):
    ...
    このテーブルは監査目的にもよく使われる。
    IEでは、管理者が注文に変更を加えるたびに、何が起こったのかを記録するノートを生成する。
    '''
...
class AbstractCommunicationEvent(models.Model):
    """
    確認メールが送信されるなど、顧客への連絡を含む注文レベルのイベントテーブル。
    """
    order = models.ForeignKey(
        'order.Order',
        on_delete=models.CASCADE,
    ...
    class Meta:
        ...
        verbose_name_plural = _("顧客対応状況")   # ←こんな認識・・？
...
class AbstractLine(models.Model):
    ...
    def all_statuses(cls):
        '''
        注文明細に対して可能なすべてのステータスを返す
        '''
        return list(cls.pipeline.keys())

    def available_statuses(self):
        '''
        この注文明細行の移動可能なすべてのステータスを返します
        '''
        return self.pipeline.get(self.status, ())
    ...
    def payment_event_quantity(self, event_type):
        """
        渡されたタイプの支払いイベントに関与したこのレコードの数量を返すメソッド。
        """
...
class AbstractPaymentEvent(models.Model):
    ...
    * 「2行返金されました」
    ...
    class Meta:
        ...
        verbose_name_plural = _("支払イベント管理")
        ordering = ['-date_created']
...
class AbstractShippingEvent(models.Model):
    """
    イベントとは、1項目がディスパッチされるなどの行のグループに起こるもの
    """
    order = models.ForeignKey(
        'order.Order',
        on_delete=models.CASCADE,
...
class AbstractOrderDiscount(models.Model):
    """
    注文に対する割引テーブル。※本検証ではdiscountは扱わないため、検証対象外とする。
